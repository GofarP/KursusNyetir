...
<body>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="post" action="{{route('paket.edit',$paket->id_paket)}}" class="mb-5"  enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="nama" class="mt-3 mb-2">Nama Paket:</label>
                    <input type="text" name="nama" placeholder="Masukkan Nama Paket"
                        class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama',$paket->nama) }}">
                        @error('nama')
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="harga" placeholder="Masukkan Harga Paket"
                        class="form-control @error('harga') is-invalid @enderror" value="{{ old('harga',$paket->harga) }}">
                        @error('harga')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="pertemuan" placeholder="Masukkan Jumlah Pertemuan"
                        class="form-control @error('pertemuan') is-invalid @enderror" value="{{ old('pertemuan',$paket->pertemuan) }}">
                        @error('pertemuan')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('sim') is-invalid @enderror" name="sim" id="sim">
                        <option value="Ya" {{$paket->sim == "Ya" ? 'selected' : ''}}>Ya</option>
                        <option value="Tidak" {{$paket->sim == "Tidak" ? 'selected' : ''}}>Tidak</option>
                    </select>
                        @error('sim')
                            <div class="invalid-feedback">
                                {{ $message }}
